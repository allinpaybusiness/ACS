...
class CreditScore:
    ...
    def __init__(self, dataname):
        ...
        if self.dataname == 'UnionPay':
            self.data = pd.read_csv('..\\UnionPay.csv', encoding="gbk")
            irrelevant = pd.read_csv('..\\UnionPayirrelevant.csv', encoding="gbk")
            
            self.data = self.data[self.data['借贷标记']!='no data'] 
            self.data = self.data[self.data['风险得分']==self.data['风险得分']]
            self.data = self.data[self.data['风险得分'] < 9990]
            del self.data['风险得分分类']
#            self.data.iloc[:,10] = self.data.iloc[:,10].replace({9990:0, 9991:1000})
#            self.data.iloc[:,11:22] = self.data.iloc[:,11:22].replace({9990:-1, 9991:11, 9992:0, 9993:0})

            for col in irrelevant.columns:
                del self.data[col]
...
#                    del self.data[col]
            ...
            self.data = self.data.replace({-9999999.0:10})
            
        if self.dataname == 'gmsc':
            self.data = pd.read_csv('cs-training.csv', encoding="gbk")
            self.testdata = pd.read_csv('cs-test.csv', encoding="gbk")

            del self.data['Unnamed: 0']
            del self.testdata['Unnamed: 0']
